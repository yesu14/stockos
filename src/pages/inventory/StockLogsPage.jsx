...
const TYPE_COLORS = {
  manual: 'bg-yellow-500/15 text-yellow-400',
  inbound: 'bg-emerald-500/15 text-emerald-400',
  outbound: 'bg-orange-500/15 text-orange-400',
  ...
  async function loadLogs() {
    ...
    let q = supabase
      ...
      .select(`
        ...
        option_label, user_name, created_at, change_type, note,
        product_skus(
          products(name),
    ...
    if (typeFilter !== 'all') q = q.eq('change_type', typeFilter)
  ...
  return (
    <div className="max-w-6xl space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">재고 변경 이력</h1>
          <p className="text-surface-400 text-xs mt-0.5">전체 재고 변경 이력 · 최근 500건</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="상품명, 옵션, 담당자 검색..."
        ...
        <div className="flex gap-1.5 flex-wrap">
          {[['all', '전체'], ...Object.entries(TYPE_LABELS)].map(([k, label]) => (
            <button key={k} onClick={() => setTypeFilter(k)}
              className={'px-3 py-2 rounded-xl text-xs font-medium transition-colors ' +
                (typeFilter === k ? 'bg-primary-500 text-white' : 'bg-surface-800 text-surface-400 hover:text-white')}>
              {label}
            </button>
          ))}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-surface-800/50 border-b border-surface-800">
                ...
                <th className="px-4 py-3 text-center text-xs font-semibold text-surface-400 uppercase">유형</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-surface-400 uppercase">담당자</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-surface-400 uppercase">메모</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-surface-400 uppercase">날짜</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-surface-800">
              {loading ? (
                <tr><td colSpan={9} className="py-16 text-center"><div className="w-7 h-7 border-2 border-primary-500 border-t-transparent rounded-full animate-spin mx-auto" /></td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={9} className="py-16 text-center text-surface-500 text-sm">기록이 없습니다</td></tr>
              ) : filtered.map(log => {
                const change = (log.quantity_after ?? 0) - (log.quantity_before ?? 0)
                const typeLabel = TYPE_LABELS[log.change_type] || log.change_type
                const typeColor = TYPE_COLORS[log.change_type] || 'bg-surface-700 text-surface-400'
                return (
                  <tr key={log.id} className="hover:bg-surface-800/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center font-mono text-surface-400">{log.quantity_before ?? '-'}</td>
                    <td className="px-4 py-3 text-center font-mono font-bold text-white">{log.quantity_after ?? '-'}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`font-mono text-sm font-bold ${change > 0 ? 'text-emerald-400' : change < 0 ? 'text-red-400' : 'text-surface-500'}`}>
                        {change > 0 ? `+${change}` : change}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-surface-300 text-xs">{log.user_name || '-'}</td>
                    <td className="px-4 py-3 text-surface-500 text-xs max-w-32 truncate">{log.note || '-'}</td>
